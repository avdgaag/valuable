...
class DeepFreezeTest < Minitest::Test
  def test_it_freezes_strings
    assert Valuable::DeepFreeze.deep_freeze('string').frozen?
  end

  def test_it_freezes_hashes
    attr = { title: 'string' }
    frozen_attr = Valuable::DeepFreeze.deep_freeze(attr)
  ...
  def test_it_freezes_arrays
    assert Valuable::DeepFreeze.deep_freeze(['a', 'b']).frozen?
  end

  def test_it_freezes_arrays_values
    arr = ['a', 'b']
    frozen_arr = Valuable::DeepFreeze.deep_freeze(arr)
    assert frozen_arr[0].frozen?
    assert frozen_arr[1].frozen?
  end

  def test_it_reuses_symbols
    assert_equal :symbol, Valuable::DeepFreeze.deep_freeze(:symbol)
  end

  def test_it_freezes_ranges
    assert Valuable::DeepFreeze.deep_freeze('a'..'z').frozen?
  end

  def test_it_freezes_ranges
    a = 'a'
    z = 'z'
    frozen_range = Valuable::DeepFreeze.deep_freeze(a..z)
